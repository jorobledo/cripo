...
def _find_table_limit(
    values: list[int], maximum: int, default_limit: int, fallback_limit: float
) -> tuple[int, float]:
...
def _append_peak(
    energies: list[float], weights: list[float], energy: float, weight: float
) -> None:
...
def _multi2_hex(
    a: float, c: float, elim: float, indic: int, idim: int, ier: int
) -> Multi2Result:
    ...
    table_limit, scaled_limit = _find_table_limit(
        I1_VALUES, int(scaled_limit), default_limit=36, fallback_limit=97.0
    )
    ...
    for base_index, base_weight in zip(
        I1_VALUES[:table_limit], IWHEX_VALUES[:table_limit]
    ):
        ...
        for l3 in range(0, l3_limit, l3_step):
            ...
            if l3 == 0:
                multi //= 2
            _append_peak(
                energies, weights, scaled_energy * energy_scale, multi / atom_divisor
            )
...
def _multi2_cub(a: float, elim: float, indic: int, idim: int, ier: int) -> Multi2Result:
    ...
    for is_value in range(1, max_index + 1):
        ...
        if multiplicity == 0:
            continue
        _append_peak(
            energies, weights, is_value * energy_scale, multiplicity * weight_scale
        )
        if len(energies) >= idim:
            break
...
def _multi2_tetr(a: float, c: float, elim: float, idim: int, ier: int) -> Multi2Result:
    ...
    scaled_limit = elim / energy_scale
    table_limit, scaled_limit = _find_table_limit(
        ISTETR_VALUES, int(scaled_limit), default_limit=43, fallback_limit=98.0
    )
    ...
    for base_index, base_weight in zip(
        ISTETR_VALUES[:table_limit], IWTETR_VALUES[:table_limit]
    ):
        if scaled_limit < base_index:
            continue
        ...
        for l3 in range(l3_limit):
            ...
            if scaled_energy <= 0.0 or scaled_energy > scaled_limit:
                continue
            _append_peak(
                energies, weights, scaled_energy * energy_scale, base_weight * iu
            )
            if len(energies) >= idim:
                return Multi2Result(
                    energies,
